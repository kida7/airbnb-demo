...
interface String {
    ...
    toRegex: (option?: string | undefined) => RegExp,
    toRegexString: (option?: string | undefined) => string,
...
String.prototype.toRegexString = function (option: string | undefined = 'i') {
    let regexStr = this.replace(/[\.\*\+\?\^\$\{\}\(\)\|\[\]\\\/]/g, '\\$&')
    regexStr = regexStr.replace(/\n/g, '\\n').replace(/ +/g, ' *')
    return regexStr
}
String.prototype.toRegex = function (option: string | undefined = 'i') {
    let regexStr = this.replace(/[\.\*\+\?\^\$\{\}\(\)\|\[\]\\\/]/g, '\\$&')
    regexStr = regexStr.replace(/\n/g, '\\n').replace(/ +/g, ' *')
    return RegExp(regexStr, option)
}
